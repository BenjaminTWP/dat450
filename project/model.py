...
def translate_tokens_batch(model, source_lang_tokens, tokenizer, device, max_length=50):
    ...
    for _ in range(max_length):
        ...
        logits = model(source_lang_tokens, target_lang_ids)
...
#     # Truncate at first EOS for each sequence
#     decoded = []
#     for i in range(batch_size):
#         tokens = target_lang_ids[i]
#         # Find first EOS
#         eos_positions = (tokens == tokenizer.eos_token_id).nonzero(as_tuple=True)[0]
#         if len(eos_positions) > 0:
#             tokens = tokens[:eos_positions[0] + 1]  # Include EOS
